...
def find_links(link,cate,c_time,m):
    try :
        print link
        output=""
        ...
        for e in c:
            ...
            try:
                l=e['href']
                if l not in output:
                    if l!=link and l!=domain_name and domain_name in l:
                        output=output+l+'\n'
                    else:
                        if l[0]=="/":
                            output=output+domain_name+l+"\n"
        ...
        if len(output)>0:
            ...
            path="D://Thesis//data//domain_name//main_category_crawler//"+link.split("http://")[1].replace("/","#")
            ...
            pr_link="D://Thesis//data//domain_name/main_base_categories_50//main//valid_links3.txt"
            f_pr=open(pr_link+" "+m,'a+')
            f_pr.write(link+'\n')
            f_pr.close()
        else:
            print "no links found invalid",link
            inv_path="D://Thesis//data//domain_name//main_category_crawler//invalid_links2.txt"
...
def time_call():
    ...
    while current_time<stop_time:
            ...
            for i in range(29579,len(sources)):
                st = datetime.datetime.now().strftime("%A, %d. %B %Y %I.%M")
                ...
                #print sources[i]
                try:
                    find_links(sources[i],cate,st,i)
                    print "proceed is",i
                    print st
                except:
                    pass
